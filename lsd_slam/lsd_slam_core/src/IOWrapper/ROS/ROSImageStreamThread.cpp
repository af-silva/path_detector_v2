...
void ROSImageStreamThread::setCalibration(std::string file)
{
	if(file == "")
	{
		...
		ROS_INFO("ROSImageStreamThread subscribed to [Camera Info] %s", info_sub.getTopic().c_str());
...
void ROSImageStreamThread::vidCb(const sensor_msgs::ImageConstPtr img)
{
	...
	// af-silva I was here
	bufferItem.frame_id = boost::lexical_cast<uint32_t>(cv_ptr->header.frame_id.c_str());
