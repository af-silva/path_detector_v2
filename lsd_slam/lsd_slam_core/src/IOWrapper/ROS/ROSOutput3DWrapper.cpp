...
ROSOutput3DWrapper::ROSOutput3DWrapper(int width, int height)
{
  ...
  imageToPathDetectorRGB = cv::Mat(height,width,CV_8UC3);
  ...
  publishLvl=0;
	...
	depthmap_channel = nh_.resolveName("lsd_slam/depthmap");
	depthmap_publisher = nh_.advertise<sensor_msgs::Image>(depthmap_channel,1);

	image_channel = nh_.resolveName("lsd_slam/image");
	image_publisher = nh_.advertise<sensor_msgs::Image>(image_channel,1);


	keyframes.set_capacity(50);

}

ROSOutput3DWrapper::~ROSOutput3DWrapper()
{
    imageToPathDetectorRGB.release();
}
...
void ROSOutput3DWrapper::publishKeyframe(Frame* f)
{
  ...
  const float *frameImageRGB = f->imageRGB();
  int rgb=0;
  for(int row=0; row < h; ++row)
  {
    for(int col=0; col < w; col++)
    {
      setPixelInCvMat(&imageToPathDetectorRGB, getRGB2CvPixel(*(frameImageRGB + rgb), *(frameImageRGB + rgb + 1), *(frameImageRGB + rgb + 2)), col, row, 1);
      rgb+=3;
    }
  }
  ...
  iMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", imageToPathDetectorRGB).toImageMsg();
  fMsg.image = *iMsg;
	keyframe_publisher.publish(fMsg);
	//image_publisher.publish(iMsg); // TODO - af-silva estava a ser publicada aqui
	iMsg.reset();
...
void ROSOutput3DWrapper::pulbishDepthMap(cv::Mat& depth_map)
{

  sensor_msgs::ImagePtr msg;
  msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", depth_map.clone()).toImageMsg();
  depthmap_publisher.publish(msg);
  msg.reset();
...
void ROSOutput3DWrapper::publishPointCloud(KFPointCloud* kf)
{
  try
  {
    float my_scaledTH = 1;
    ...
    float my_scale = kf->camToWorld.scale();
    int my_minNearSupport = 5;
    int my_sparsifyFactor = 1;
    float fxi = 1/kf->fx;
    float fyi = 1/kf->fy;
    float cxi = -kf->cx / kf->fx;
    float cyi = -kf->cy / kf->fy;
    ...
    for(int y=0; y < kf->height; y++)
    {
      for(int x=0; x < kf->width; x++)
      {
       ...
       if (x == 0 || y == 0 || x == kf->width - 1 || y == kf->height - 1)
       {
           kf->pointCloud->push_back(point_invalid_);
           continue;
       }
        if(kf->originalInput[x+y*kf->width].idepth <= 0)
        {
          kf->pointCloud->push_back(point_invalid_);
          continue;
        }
        ...
        float depth = 1 / kf->originalInput[x+y*kf->width].idepth;
        float depth4 = depth*depth; depth4*= depth4;
        if(kf->originalInput[x+y*kf->width].idepth_var * depth4 > my_scaledTH)
        {
          kf->pointCloud->push_back(point_invalid_);
          continue;
        }
        if(kf->originalInput[x+y*kf->width].idepth_var * depth4 * my_scale*my_scale > my_absTH)
        {
          kf->pointCloud->push_back(point_invalid_);
          continue;
        }
        if(my_minNearSupport > 1)
        {
          ...
          for(int dx=-1;dx<2;dx++)
            ...
            {
              int idx = x+dx+(y+dy)*kf->width;
              if(kf->originalInput[idx].idepth > 0)
              {
                float diff = kf->originalInput[idx].idepth - 1.0f / depth;
                if(diff*diff < 2*kf->originalInput[x+y*kf->width].idepth_var)
                  nearSupport++;
              }
            }
        ...
        //Sophus::Vector3f pt = kf->keyFrameCamToWorld * (Sophus::Vector3f((x*kf->fxi + kf->cxi), (y*kf->fyi + kf->cyi), 1) * depth);
        Sophus::Vector3f pt = (Sophus::Vector3f((x*fxi+cxi), (y*fyi+cyi), 1) * depth);
        point_pcl.x = pt[0];
        point_pcl.y = pt[1];
        point_pcl.z = pt[2];
        point_pcl.r = kf->originalInput[x+y*kf->width].color[0];
        point_pcl.g = kf->originalInput[x+y*kf->width].color[1];
        point_pcl.b = kf->originalInput[x+y*kf->width].color[2];

        kf->pointCloud->push_back(point_pcl);
        displayed++;
      }
    }

    kf->pointCloud->width = kf->width;
    kf->pointCloud->height = kf->height;
    kf->pointCloud->is_dense = false;
    kf->pointCloud->header.frame_id = "frame";
    pcl_conversions::toPCL(ros::Time::now(), kf->pointCloud->header.stamp);


    kf->totalPoints = total;
    kf->displayedPoints = displayed;


    sensor_msgs::PointCloud2 output;
    pcl::toROSMsg(*kf->pointCloud, output);
