...
void LiveSLAMWrapper::Loop()
{
	while (true) {
		...
		// af-silva aqui a imagem e enviada para a funcao que faz o primeiro tratamento
		// process image
		//Util::displayImage("MyVideo", image.data);
		newImageCallback(image.data, image.timestamp, image.frame_id);
	}
}
 ...
 *
 */
void LiveSLAMWrapper::newImageCallback(const cv::Mat& img, Timestamp imgTime, int frame_id)
{
	++ imageSeqNumber;
	...
	else if(isInitialized && monoOdometry != nullptr)
	{
		...
		monoOdometry->trackFrame(grayImg.data, img.data, imageSeqNumber, false, imgTime.toSec(), frame_id);
	}
}
