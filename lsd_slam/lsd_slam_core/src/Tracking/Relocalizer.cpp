...
//			currentFrame->id(), nextRelocIDX, maxRelocIDX);
	...
	if (displayDepthMap)
		Util::displayImage( "DebugWindow DEPTH Relocalization", cv::Mat(currentFrame->height(), currentFrame->width(), CV_32F, currentFrame->image())*(1/255.0f), false );
...
void Relocalizer::start(boost::circular_buffer<Frame*, Eigen::aligned_allocator<lsd_slam::Frame*> > &allKeyframesList)
{
