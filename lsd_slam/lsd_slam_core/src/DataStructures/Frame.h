...
public:
	...
	Frame(int id, int real_id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image, const unsigned char* rgbImage);
...
private:
	...
	void releaseRGBImage(int level);
...
inline int Frame::frame_id() const
{
  return data.real_id;
}
