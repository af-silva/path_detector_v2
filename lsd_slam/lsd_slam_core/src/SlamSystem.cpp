...
// af-silva see this part of the code
void SlamSystem::debugDisplayDepthMap()
{
	...
	outputWrapper->pulbishDepthMap(map->debugImageDepth);

	if(onSceenInfoDisplay)
		printMessageOnCVImage(map->debugImageDepth, buf1, buf2);
	if (displayDepthMap)
		Util::displayImage( "DebugWindow DEPTH SLAM SYSTEM", map->debugImageDepth, false );
...
void SlamSystem::trackFrame(uchar* image, uchar* rgbImage, unsigned int frameID, bool blockUntilMapped, double timestamp, unsigned int frame_id)
{
	// Create new frame
	//std::shared_ptr<Frame> trackingNewFrame(new Frame(frameID, width, height, K, timestamp, image));
	std::shared_ptr<Frame> trackingNewFrame(new Frame(frameID, frame_id, width, height, K, timestamp, image, rgbImage));
...
boost::circular_buffer<FramePoseStruct*, Eigen::aligned_allocator<FramePoseStruct*> > SlamSystem::getAllPoses()
{
